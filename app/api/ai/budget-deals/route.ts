...
export async function POST(req: NextRequest) {
  try {
    ...
    const productListStr = ads.map((ad: any) => `
- ID: ${ad._id.toString()}
- Title: ${ad.title}
- Price: ₹${ad.price}
...
`).join("\n");
    ...
    if (apiKey) {
      try {
        ...
        console.warn("Falling back to Smart Keyword Mode due to:", error.message);
        isMock = true;
      }
    } else {
      console.warn("No GEMINI_API_KEY — using smart keyword fallback.");
      isMock = true;
    }

    // Fallback: Smart Keyword Matching + Value Ranking
    if (isMock || recommendations.length === 0) {
      ...
      recommendations = ads
        .map(ad => {
          ...
          // Budget proximity (closer to budget = better, slightly under = best)
        ...
        .map(({ ad }) => ({
          ...
          reason: `Market Insight: This ${ad.category} item offers great value at ₹${ad.price.toLocaleString("en-IN")} — competitively priced for its category and matches your search for "${query || "quality products"}".`
        }));
    }
